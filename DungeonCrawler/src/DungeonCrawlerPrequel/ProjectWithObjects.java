 ...
 * @author mfaux02
 */
public class ProjectWithObjects {
    ...
    public static void doGameStart(){
        ...
        while(running){
            if(player.health < 1){
                ...
                switch(resp){
                    case 'y': player.createStats(player.level, player.Name);
                    player.level = 0;
                    realizeCharacter();
                ...
                switch(response){
                    case "heal":
                        int healing = (int)(player.healthMax - player.health);
                        ...
                        if(response.charAt(0) == 'y' & player.coinage >= healing/50){
                            player.health = player.healthMax;
                            player.coinage -= healing/50;
                        }//close if
                        else if (player.coinage < healing/50){
    ...
    public static void fight(int action){
        ...
        double toHit = (randGen.nextInt(76) + player.acc * .25 - player.level * 10) / (randGen.nextInt(51) + 25 + mon.dex * .25 - mon.level * 10);
        double toDodge = (randGen.nextInt(76) + mon.acc * .25 - mon.level * 10) / (randGen.nextInt(51) + 25 + player.dex * .25 - player.level * 10 + dodge);
        
        if(toHit >= 1){
            double damageGiven = randGen.nextInt(26) + player.str * .25 + 5  - player.level * .25 + heavyAttack;
            double critChance = randGen.nextInt(21)  + player.cri * .25 - player.level ;
            if(critChance > 100){
                damageGiven *= 2;
